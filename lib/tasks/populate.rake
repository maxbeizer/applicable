namespace :db do
  task :populate => :environment do
    ...
    3.times do |i|
      student = Student.new :email => "test#{i}@example.com", :password => 'password', :password_confirmation => 'password',
                            :first_name => "test #{i} first", :last_name => "test #{i} last",
                            :date_of_birth => "2013-05-#{i+1}", :phone => '123-123-1234', :twitter => '@handle',
